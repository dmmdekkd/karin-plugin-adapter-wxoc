		...
		return [components.accordionPro.create("accounts", cfg.accounts.map((account) => ({
			...
			children: components.accordion.createItem("account", {
				...
				children: [
					...
					components.input.password("token", {
						label: "登录凭证",
						color: "danger"
					}),
					components.input.string("accountId", {
						label: "iLink Bot ID",
						color: "danger"
					}),
					components.input.string("userId", {
						label: "用户 ID",
						color: "danger"
					}),
					...
					components.input.url("baseUrl", {
						label: "API 地址",
						color: "danger"
					}),
					components.switch.create("isDisable", {
						label: "禁用",
						color: "danger"
					})
				]
			...
			children: [
				components.accordion.createItem("server", {
					...
					children: [
						...
						components.input.url("cdnUrl", {
							label: "CDN 地址",
							color: "danger",
							defaultValue: cfg.cdnUrl
						}),
						components.input.string("botType", {
							label: "机器人类型",
							color: "danger",
							defaultValue: cfg.botType
						})
					]
				}),
				...
				components.accordion.createItem("switches", {
					...
					subtitle: "行为开关与自动更新",
					children: [
						components.switch.create("downloadFile", {
							label: "接收文件自动下载",
							color: "danger",
							defaultSelected: cfg.downloadFile
						}),
				...
				components.accordion.createItem("avatar", {
					title: "头像",
					subtitle: "Bot 与用户头像",
					children: [components.input.url("botAvatar", {
						label: "Bot 头像地址",
						color: "danger",
						defaultValue: cfg.botAvatar
					}), components.input.url("userAvatar", {
						label: "用户头像地址",
						color: "danger",
						defaultValue: cfg.userAvatar
					})]
				}),
